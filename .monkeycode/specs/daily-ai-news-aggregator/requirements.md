...
本工具是一个AI资讯聚合与智能分析平台，从多个权威来源抓取AI相关资讯，支持手动触发和定时抓取两种模式。平台提供数据存储、汇总分析、技术趋势图表等功能，帮助用户高效追踪AI领域动态。
...
**用户故事：** AS 一个分析师，我想要系统对资讯进行AI质量评分，以便筛选出最有价值的内容。

#### Acceptance Criteria

1. WHEN 文章抓取并去重完成后，Scorer SHALL 调用AI API对每篇文章进行质量评分（0-10分）。
2. WHERE 评分因素包括技术深度、创新性、影响力、来源权威性，AI SHALL 综合这些因素给出最终评分。
3. IF 用户配置了多个AI提供商，系统 SHALL 支持轮询或备用机制以提高稳定性。
4. WHERE AI提供商包括OpenAI、Anthropic、DeepSeek、Doubao、MiniMax，系统 SHALL 支持切换。
5. IF 文章AI评分低于配置的阈值（默认6.0分），系统 SHALL 标记为低质量但不自动删除。
6. WHEN 评分完成后，系统 SHALL 将评分结果存储到数据库的score字段。
...
1. WHERE 热度计算考虑AI评分（权重40%）、来源权重（权重20%）、发布时间（权重20%）、互动指标（权重20%），HotnessRanker SHALL 计算综合热度分数。
2. WHERE 来源权重由管理员配置，系统 SHALL 支持调整各来源的权重系数。
3. WHERE 互动指标包括评论数、点赞数、转发数，系统 SHALL 从各平台抓取这些数据。
...
2. WHERE 配置包含AI提供商设置时，系统 SHALL 支持 OpenAI/Anthropic/Gemini/DeepSeek 等提供商。
3. IF 用户执行 `horizon wizard` 命令，系统 SHALL 启动交互式配置向导。
4. WHERE 配置包含过滤阈值时，文章 SHALL 仅保留AI评分高于阈值的内容（如果启用了AI评分）。
